...
public class OrderCancelRepairJobTest
{
	...
	public void cronJobTest()
	{
		...
		when(orderExchangeRepair.findProcessesByActionIds(Flintyb2bsaporderfulfillmentConstants.ORDER_PROCESS_NAME, actions)).thenReturn(
				bpList);
		repairJob.perform(cronJobModel);
		verify(orderExchangeRepair).findProcessesByActionIds(Flintyb2bsaporderfulfillmentConstants.ORDER_PROCESS_NAME, actions);
	}
